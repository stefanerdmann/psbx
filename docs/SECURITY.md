...
This document describes the trust boundaries pi-sandbox enforces, what is explicitly out of scope, and how host data flows into and out of the VM.

## Trust model

pi-sandbox assumes **the host is trusted and the VM guest is untrusted**. The design prevents the guest from modifying host state beyond the project's own directory.

### Trust boundaries

| Boundary | Enforcement |
|---|---|
| Host profile config dirs → VM | Each subfolder declared in `env.yaml#configMounts` is mounted **read-only** at `/mnt/host-config/<name>`. pi-sandbox copies the contents into the corresponding guest target (e.g., `~/.pi/agent`, `~/.copilot`) during finalization but the VM cannot write back. |
...
| `.agents` directory in project | pi-sandbox refuses to create state inside `.agents` if it is a symlink, preventing symlink-based path traversal. |
| Profile cache VM → project VM | The hidden cache VM is built without project mounts or profile config mounts. Before it is stopped for cloning, pi-sandbox removes clone-sensitive guest identity files such as SSH host keys and machine-id. |
...
Host                                    VM Guest
─────────────────────────────────────── ───────────────────────────────
~/.pi-sandbox/profiles/<p>/<configMount.source> ──► /mnt/host-config/<name> (RO)
                                                  └─ copied to <guestTarget> (VM-local)
<project>/ ◄────────────────────────►   ~/workdir (RW)
                                        └─ ~/workdir/.agents/sessions (persistent)
...
pi-sandbox is **not** a security sandbox in the container-hardening sense. The following are explicitly out of scope:

| Non-goal | Rationale |
|---|---|
| Protection against a malicious guest escaping the VM | Lima VMs rely on the host hypervisor (Apple Virtualization.framework, QEMU). pi-sandbox adds no additional confinement beyond what Lima provides. |
...
| Multi-tenant isolation | pi-sandbox is designed for single-user developer workstations, not shared infrastructure. |
...
- **Keep cache provisioning generic.** Do not copy secrets or project data in profile provisioning scripts. Let pi-sandbox finalization copy profile config into each project VM after cloning.
...
- **Use project overrides sparingly.** A project's `.pi-sandbox/lima.yaml` can only adjust resource limits (`cpus`, `memory`, `disk`), which is safe. Do not relax this allowlist.
