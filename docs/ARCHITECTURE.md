...
Each agent's config directory (e.g., `~/.pi/agent`, `~/.copilot`) is copied into the VM during finalization and is VM-local. `~/workdir/.agents` is inside the writable project mount and persists on the host. When a `configMount` declares both `sessions.workspacePath` and `sessions.guestSymlink`, finalization creates the project directory and replaces the `sessions.guestSymlink` path inside the VM with a symlink pointing to it — for example, `~/.pi/agents/sessions` → `~/workdir/.agents/pi-sessions` and `~/.copilot/session-state` → `~/workdir/.agents/copilot-sessions/session-state`.
